...
**Addressing Known RSSI Limitations** (per literature):
- **MAC Rotation Challenge**: Apple BLE devices rotate MAC addresses every 15-24 hours 
  for privacy (CUJO AI, 2024). We address this via RSSI fingerprint clustering (DBSCAN, 
  ε=3 dBm) to identify logical devices across MAC changes.
  
- **RSSI Variance**: Indoor BLE RSSI exhibits σ=3-8 dBm due to multipath, interference, 
  and environmental factors (Indoor Asset Tracking Guide, 2024). We mitigate via:
  - Rolling median filtering (50-packet window)
  - Statistical aggregation (median RSSI vs single measurements)
  - Error bound quantification (±35% distance error acknowledged)

- **Positioning Accuracy**: RSSI-to-distance conversion accurate to ±2-3 meters for 
  room-scale deployments. Our analysis focuses on **zone-level terrain mapping** 
  (not precision tracking), where this accuracy suffices.
...
- Privacy-preserving: Exploits aggregate spatial signatures, not personal data
